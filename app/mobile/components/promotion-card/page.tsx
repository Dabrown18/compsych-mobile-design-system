...
// Control the progress bar and ring timer (0–1)
<PromotionCard usage="neutral" chipLabel="Offer" title="Limited time" progress={0.7} showProgressBar showRingTimer />`} language="tsx" />
      </Section>

      <Section heading="Filled Variants" lead="Dark/saturated backgrounds — white text on all coloured usages.">
        <Surface>
      ...
      <Section heading="Tonal Variants" lead="Light container backgrounds — on-container text for all coloured usages.">
        <Surface>
          <div className="flex flex-col gap-4 w-full max-w-sm">
            {USAGES.filter((u) => u !== 'image').map((u) => (
              <div key={u} className="flex flex-col gap-1">
