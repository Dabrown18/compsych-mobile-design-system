...
export const PRODUCTS = ['gro', 'crc', 'absence-resources'] as const;
...
export const PRODUCT_LABELS: Record<Product, string> = {
  ...
  'absence-resources': 'Absence Resources',
};
...
export interface TokenLeaf {
  path: string[];              // e.g. ['color', 'primary'] or ['typography', 'bodyLarge', 'fontSize']
  cssVar: string;              // e.g. '--sys-color-primary'
 ...
 * `sys.color` to scope to one branch. The `rootPath` argument preserves
 * the ancestor path (needed so the CSS variable name lines up with the
 * full `sys.*` path).
 */
